...
$(document).ready(function() {
    ...
    var add_stock_location = function() {
        ...
        $(new_block).find('label').html("<?php echo $this->lang->line('config_stock_location'); ?> " + ++
            location_count).attr('for', new_block_id).attr('class', 'control-label col-xs-2');
        $(new_block).find('input').attr('id', new_block_id).removeAttr('disabled').attr('name',
            new_block_id).attr('class', 'form-control input-sm').val('');
        hide_show_remove();
    };
        ...
        submitHandler: function(form) {
            ...
            $(form).ajaxSubmit({
                success: function(response) {
                    ...
                    $("#stock_locations").load(
                        '<?php echo site_url("config/ajax_stock_locations"); ?>',
                        init_add_remove_locations);
                },
                dataType: 'json'
            });
